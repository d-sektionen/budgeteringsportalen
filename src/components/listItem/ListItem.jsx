...
const ListItem = ({ doc, updateOverview }) => {
	...
	return (
		<>
			<Modal isOpen={isOpen} contentLabel='Item' style={customStyles}>
				<div
					...
					<div>
						<label
							className={`statusLabel ${
								doc.approvedKas ? 'approved' : 'disabled'
							}`}
						>
							Attesterat <FaCheckCircle />
						</label>
						<label
							className={`statusLabel ${
								doc.approvedDeg ? 'approved' : 'disabled'
							}`}
						>
							Bokfört <FaBook />
						</label>
						<label
							className={`statusLabel ${doc.payed ? 'approved' : 'disabled'}`}
						>
							Betalt <FaDollarSign />
						</label>
					</div>
